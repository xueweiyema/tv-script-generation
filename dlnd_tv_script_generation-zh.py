...
def create_lookup_tables(text):
    ...
    words=set(text)
    vocab_to_int = {vocab: i for i,vocab in enumerate(words)}
    int_to_vocab = {i: vocab for i,vocab in enumerate(words)}
...
def get_init_cell(batch_size, rnn_size):
    ...
    """
    # TODO: Implement Function
    basiclstmcells=tf.contrib.rnn.BasicLSTMCell(rnn_size)
    multirnncell=tf.contrib.rnn.MultiRNNCell([basiclstmcells]*2)
    
    InitialState=multirnncell.zero_state(batch_size,tf.float32)
    InitialState=tf.identity(InitialState,name='initial_state')
    
    return (multirnncell, InitialState)
...
def get_embed(input_data, vocab_size, embed_dim):
    ...
    #  random_uniform/truncated_normal/radom_normal/random_shuffle   
...
np.zeros([5,2,2,3],dtype=np.int32)
...
def get_batches(int_text, batch_size, seq_length):
    ...
    input_data = np.array(int_text[:batch_size*seq_length*n_batch])
    target_data = np.array(int_text[1:batch_size*seq_length*n_batch+1])
    input_batches = np.split(input_data.reshape(batch_size, -1), n_batch, 1)
    target_batches = np.split(target_data.reshape(batch_size, -1), n_batch, 1)
    
    l=[]
    for i in range(len(input_batches)):
        l.append([input_batches[i],target_batches[i]])
    print(l)
#     return l
    return np.array(l)
